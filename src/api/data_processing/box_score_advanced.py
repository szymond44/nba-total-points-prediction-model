...
class BoxScoreAdvancedProcessing(DataProcessing):
    ...
    def __init__(self, **kwargs):
        ...
        self.__cache_file = self.__cache_dir / f"boxscore_{self.__starting_year}_{self.__ending_year}.json"
        self.__lock = Lock()
        self.__cache = {}
        self.__load_cache()
    ...
    def __process_seasons(self, starting_year: int, ending_year: int):
        all_games = []
        processing = LeagueGameLogProcessing(starting_year=starting_year, ending_year=ending_year, all=True)
        ...
        print(f"🔄 Total games: {len(games)}, Cached: {len(games) - len(games_to_process)}, To process: {len(games_to_process)}")
    ...
    def __process_games_parallel(self, games_to_process):
        ...
        try:
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                with tqdm(total=len(games_to_process), desc="Processing games", ncols=120) as pbar:
                    ...
                    for i in range(0, len(games_to_process), batch_size):
                        ...
                        future_to_game = {
                            executor.submit(self.__process_single_game_safe, game_id): game_id 
                        ...
                        for future in as_completed(future_to_game, timeout=60):
                            ...
                            try:
                                ...
                                success_rate = ((processed_count - errors_count) / processed_count * 100) if processed_count > 0 else 0
                                pbar.set_postfix({
                                ...
                                print(f"⏰ Timeout/Error for {game_id}: {e}")
                                errors_count += 1
                                processed_count += 1
                                pbar.update(1)
                        ...
                        if not self.__stop_event.is_set() and i + batch_size < len(games_to_process):
    ...
    def get_cache_stats(self):
        return {
            ...
            "cache_size_mb": self.__cache_file.stat().st_size / (1024*1024) if self.__cache_file.exists() else 0
        }
    ...
    def __convert_numpy_types(self, data):
        if isinstance(data, list):
            ...
            return {key: self.__convert_numpy_types(value) for key, value in data.items()}
